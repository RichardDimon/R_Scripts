...
run_dart_cleaning_loop <- function(
  ...
  reanalyse <- function(d1_current, dms_current, round_tag) {
    ...
    qc1 <- report.dart.qc.stats(d1_new, RandRbase, species, dataset, threshold_missing_loci = 0.8)
    d2_new <- remove.poor.quality.snps(d1_new, min_repro=0.96, max_missing=locus_miss)
    qc2 <- report.dart.qc.stats(d2_new, RandRbase, species, dataset)
    d3_new <- sample.one.snp.per.locus.random(d2_new, seed=214241)
    d3_new$treatment <- paste0(d3_new$treatment, "_rep", round_tag)
    qc3 <- report.dart.qc.stats(d3_new, RandRbase, species, dataset)
  ...
  while (TRUE) {
    ...
    dms <- disambiguate_site_names(dms)
    kin_final <- individual_kinship_by_pop(dms, RandRbase, species, dataset, dms$meta$analyses[,site_col_name], maf=maf_val, mis=locus_miss, as_bigmat=TRUE)
