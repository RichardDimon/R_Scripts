...
downsample <- setup_variables[10, 2]
samples_per_pop_downsample <- setup_variables[11, 2] %>% as.numeric()
locus_miss <- setup_variables[12, 2] %>% as.numeric()
sample_miss <- setup_variables[13, 2] %>% as.numeric()
maf_val <- setup_variables[14, 2] %>% as.numeric()
clonal_threshold <- setup_variables[15, 2] %>% as.numeric()
...
d3$treatment <- paste0(d3$treatment,"_rep1")
qc3       <- report.dart.qc.stats(d3,RandRbase, species, dataset, threshold_missing_loci = 0.5)
...
write.table(data.frame(sample=samples_high_missing, sample_miss=rowMeans(is.na(dms$gt))[which(rowMeans(is.na(dms$gt))>sample_miss)]), 
            paste0(species,"/outputs_",site_col_name,"_",species_col_name,"/tables/high_missing_samples_removed_round1.tsv"), sep="\t", row.names = FALSE)
...
if (!length(d1$sample_names)==length(dms$sample_names)){
  ...
  qc2       <- report.dart.qc.stats(d2, RandRbase, species, dataset)
  d3        <- sample.one.snp.per.locus.random(d2, seed=214241)
  d3$treatment <- paste0(d3$treatment,"_rep2")
...
print(paste0("Found ", length(samples_high_missing), " samples with missingness over ",sample_miss,". removing from analysis"  ))
write.table(data.frame(sample=samples_high_missing, sample_miss=rowMeans(is.na(dms$gt))[which(rowMeans(is.na(dms$gt))>sample_miss)]), 
            paste0(species,"/outputs_",site_col_name,"_",species_col_name,"/tables/high_missing_samples_removed_round2.tsv"), sep="\t", row.names = FALSE)
dms <- remove.by.missingness(dms, sample_miss)
sites_high_missing_samples_removed <- table(dms$meta$analyses[,site_col_name])
}
...
if (!length(d1$sample_names)==length(dms$sample_names)){
  ...
  qc2       <- report.dart.qc.stats(d2, RandRbase, species, dataset)
  d3        <- sample.one.snp.per.locus.random(d2, seed=214241)
  d3$treatment <- paste0(d3$treatment,"_rep3")
  qc3       <- report.dart.qc.stats(d3, RandRbase, species, dataset)
...
#Wait a minute! - do you have the same population name applied to different species?...lets check

for (g in 1:length(unique(dms$meta$analyses[,site_col_name]))){
  numz <- which(dms$meta$analyses[,site_col_name] ==unique(dms$meta$analyses[,site_col_name])[g])
  if (length(unique(dms$meta$analyses[,species_col_name][numz]))>1){
    for(t in 1:length(unique(dms$meta$analyses[,species_col_name][numz]))){
      numz2 <- which(dms$meta$analyses[,species_col_name][numz]==unique(dms$meta$analyses[,species_col_name][numz])[t])
      dms$meta$analyses[,site_col_name][numz][numz2] <-  paste0(dms$meta$analyses[,site_col_name][numz][numz2],"_",t)
    }
  }
}
...
write.xlsx(clones_out, paste0(species,"/outputs_",site_col_name,"_",species_col_name,"/tables/PLINK_clones_round1.xlsx"), asTable = FALSE, overwrite = TRUE)
...
print(paste0("Found ", length(samples_high_missing), " samples with missingness over ",sample_miss,". removing from analysis"  ))
write.table(data.frame(sample=samples_high_missing, sample_miss=rowMeans(is.na(dms$gt))[which(rowMeans(is.na(dms$gt))>sample_miss)]), 
            paste0(species,"/outputs_",site_col_name,"_",species_col_name,"/tables/high_missing_samples_removed_round3.tsv"), sep="\t", row.names = FALSE)
dms <- remove.by.missingness(dms, sample_miss)
sites_high_missing_samples_removed <- table(dms$meta$analyses[,site_col_name])
}
...
if (!length(d1$sample_names)==length(dms$sample_names)){
  ...
  qc2       <- report.dart.qc.stats(d2, RandRbase, species, dataset)
  d3        <- sample.one.snp.per.locus.random(d2, seed=214241)
  d3$treatment <- paste0(d3$treatment,"_rep4")
  qc3       <- report.dart.qc.stats(d3, RandRbase, species, dataset)
