 ...
 if (auto_nt){
  ...
  for (o in 1:length(measurevals)){
    ...
    for ( i in 1:length(N_t_vec) ) {
      ...
      opt_results <- optimize_single_objective(gt=gt_sw_comp, sm = NULL, N_t=N_t, measure=measure, max_steps=max_steps, max_t=max_t, m=m, p_depends_delta=FALSE, q=NULL, ncpu=ncpu, weights_max = max_wts,initial_weights = initial_weights, weights_min= weights_min, unlim_m = ulimM, kinall=kinall)
